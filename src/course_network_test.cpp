...
class CourseNetworkTest : public ::testing::Test {
 public:
	 void SetUp() override {
		 ...
		 graph[english125_vertex] = Course{"ENGLISH", 125, 0};
		 auto chem210_vertex = vertex(1, graph);
		 graph[chem210_vertex] = Course{"CHEM", 210, 0};
		 auto chem211_vertex = vertex(2, graph);
		 graph[chem211_vertex] = Course{"CHEM", 211, 0};
		 auto aaptis277_vertex = vertex(3, graph);
		 graph[aaptis277_vertex] = Course{"AAPTIS", 277, 0};
		 auto environ311_vertex = vertex(4, graph);
		 graph[environ311_vertex] = Course{"ENVIRON", 311, 0};
		 auto math425_vertex = vertex(5, graph);
		 graph[math425_vertex] = Course{"MATH", 425, 0};
